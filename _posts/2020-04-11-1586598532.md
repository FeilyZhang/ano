...
对于集合$A = \{a_1, \cdots, a_n\}$，其子集可以分为两组：不包含$a_n$的子集和包含$a_n$的子集，前一组实际上就是$\{a_1, \cdots, a_{n - 1}\}$的所有子集，而后一组中的每个元素都可以通过把$a_n$添加到$\{a_1, \cdots, a_{n - 1}\}$的一个子集中来获得。因此，我们一旦得到了$\{a_1, \cdots, a_{n - 1}\}$的所有子集列表，我们就可以把列表中的每一个元素加上$a_n$，然后再整体添加至原列表中，便可得到$\{a_1, \cdots, a_n\}$的所有子集。代码如下
